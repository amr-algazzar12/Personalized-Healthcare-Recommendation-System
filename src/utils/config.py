...
Import this module in every src/ script instead of hardcoding paths.
"""
...
# Conda env Python path — used in spark-submit --conf
...
# =============================================================================
# Postgres (external_postgres_db — used by Airflow and app, NOT Hive metastore)
# =============================================================================
POSTGRES_HOST     = os.getenv("POSTGRES_HOST", "external_postgres_db")
POSTGRES_PORT     = int(os.getenv("POSTGRES_PORT", "5432"))
POSTGRES_USER     = os.getenv("POSTGRES_USER", "healthcare")
POSTGRES_PASSWORD = os.getenv("POSTGRES_PASSWORD", "healthcare")
...
# =============================================================================
# Path validation helper — catches the most common early mistake
# =============================================================================
def assert_hdfs_path(path: str) -> None:
    """
    Raises ValueError if a path does not start with the HDFS URI.
    Call this at the top of every PySpark script to catch accidental
    local-path usage before a YARN job is submitted.
    """
    if not path.startswith(HDFS_BASE):
        raise ValueError(
            f"[config] Path must be an HDFS path starting with '{HDFS_BASE}'.\n"
            f"Got: '{path}'\n"
            f"Do not use local container paths in Spark jobs."
        )